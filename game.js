...
function createSuggestions() {
    //Creating deep copy of game_option values
    suggestions_obj = JSON.stringify(game_options);   
...
function genPath(images) {
    //Displays the count for images left
    let image_count = 0;
    for (let key in game_options) {
        let values = game_options[key].length;
        image_count += values;
    }
    image_count += skipped_images.length;
    image_count > 1 ? images_left.innerText = `${image_count} images left` : images_left.innerText = `${image_count} image left`;
...
//Cetain categories of images should have different min-widths so they display better
const setWidth = (path) => {
    list = path.split('/');
...
//Refreshes browser, which restarts the game
